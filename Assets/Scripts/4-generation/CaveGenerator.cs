        ...
        for (int x = 0; x < gridSize; x++) {
            for (int y = 0; y < gridSize; y++) {
                if (x == 0 || x == gridSize - 1 || y == 0 || y == gridSize - 1) {
                    ...
                    if (rand < randomFillPercent) bufferOld[x, y] = 1;
                    else if (rand > randomFillPercent && rand < 0.7f) bufferOld[x, y] = 3;
        ...
        for (int x = 0; x < gridSize; x++) {
            for (int y = 0; y < gridSize; y++) {
                ...
                if (surroundingWalls[0] > 4) {
                    bufferNew[x, y] = 1;
                } else if(surroundingWalls[1] > 3 ) bufferNew[x, y] = 3;
                else if (surroundingWalls[0] == 4 || surroundingWalls[1] == 4) {
    ...
    private int [] GetSurroundingWallCount(int cellX, int cellY) {
        ...
        for (int neighborX = cellX - 1; neighborX <= cellX + 1; neighborX ++) {
            for (int neighborY = cellY - 1; neighborY <= cellY + 1; neighborY++) {
                ...
                if (bufferOld[neighborX, neighborY] == 1 || bufferOld[neighborX, neighborY] == 2) {
                    wallCounter[0] += 1;
                }
                else if(bufferOld[neighborX, neighborY] == 3) wallCounter[1] += 1;
            }
        }
        return wallCounter;
    }
