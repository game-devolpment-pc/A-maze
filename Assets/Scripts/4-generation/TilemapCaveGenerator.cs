...
public class TilemapCaveGenerator: MonoBehaviour {
    ...
    [Tooltip("The tile that represents a  Limits (a passable block)")]
    [SerializeField] TileBase LimitsTile = null;

    [Tooltip("The tile that represents a  Trees (a passable block)")]
    [SerializeField] TileBase TreesTile = null;
    ...
    private void GenerateAndDisplayTexture(int[,] data) {
        for (int y = 0; y < gridSize; y++) {
            for (int x = 0; x < gridSize; x++) {
                ...
                TileBase tile = null;
                if (data[x, y] == 1)  tile = wallTile;
                else if (data[x, y] == 0)  tile = floorTile;
                else if (data[x, y] == 2)  tile = LimitsTile;
                else if (data[x, y] == 3) tile = TreesTile;
